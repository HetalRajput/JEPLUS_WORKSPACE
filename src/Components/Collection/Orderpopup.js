...
import { Getuser } from '../../Constant/Api/Collectionapi/Apiendpoint';
import { placeOrder } from '../../Constant/Api/Collectionapi/Apiendpoint';
...
const OrderPopup = ({ visible, data, onClose, onConfirm, clearcart, customer }) => {
  const [updatedData, setUpdatedData] = useState(data);
  const [userDiscount, setUserDiscount] = useState(null);
  const [userCode, setUserCode] = useState(null);
  const [successModalVisible, setSuccessModalVisible] = useState(false);
  const [loading, setLoading] = useState(false); // State to track loading status
  const [smanCode, setSmanCode] = useState(null); // State for salesperson code
  useEffect(() => {
    if (customer) {
      setUserDiscount(customer.dis || 0); // Set discount, default to 0 if not provided
      setUserCode(customer.code); // Set customer code
    }
  }, [customer]);

  // Fetch user data (salesperson code) when the component mounts or customer changes
  useEffect(() => {
